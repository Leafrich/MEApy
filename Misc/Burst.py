...
falling_crossings = detect_threshold_falling_crossings(data_array[3], fs, falling_threshold, 0.001)
rising_crossings = detect_threshold_rising_crossings(data_array[3], fs, rising_threshold, 0.001)
...
spks = detect_distance_minmax(spks_fall, fs, 0.00005)
...
df22['End timestamp [µs]'] = df22['Start timestamp [µs]'] + df22['Duration [µs]']

sts = np.asarray(df22['End timestamp [µs]'])
ets = np.asarray(df22['Start timestamp [µs]'])

# for i in range(len(sts)):
#     burst_dot = np.linspace(sts[i] / 1000000, ets[i] / 1000000, 1000)
#     _ = plt.plot(burst_dot, [falling_threshold * 1e6 - 2] * 1000, 'yo', ms=0.7, alpha=0.5)

for i in range(len(bursts)):
    burst_dot = np.linspace(min(bursts[i]) / fs, max(bursts[i]) / fs, 1000)
    _ = plt.plot(burst_dot, [falling_threshold * 1e6 - 3] * 1000, 'ro', ms=0.7, alpha=0.5)
